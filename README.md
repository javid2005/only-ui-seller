...
| `/products/list` | لیست محصولات — نسخهٔ قدیم (جدول + view کارت، فیلتر، pagination، انتخاب گروهی). بدون لینک Sidebar، فقط با URL مستقیم |
| `/products/list2` | لیست محصولات — نسخهٔ جدید (KPI آیکون‌دار، فیلتر ساده‌شده + دیالوگ فیلترها، بج فیلتر فعال، فقط جدول دسکتاپ). Sidebar «لیست محصولات» به این اشاره می‌کنه |
...
| `/promotions/ads` | پروموشن‌ها — لیست کارت‌ها (خرید اول، خرید بعدی، ارسال رایگان، پیامک انبوه) + دیالوگ تنظیمات تخفیف برای دو کارت اول |
| `/promotions/free-shipping` | ارسال رایگان — انتخاب مناطق جغرافیایی + پیکربندی تخفیف و شرایط |
| `/promotions/bulk-sms` | پیامک انبوه — لیست کمپین‌ها (جدول) + اقدامات ردیف (ویرایش/حذف) + تاریخچه ارسال |
| `/promotions/abandoned-carts` | سبدهای خرید رها شده — لیست کارت‌های abandoned با جزئیات + دیالوگ نمایش سفارش + مدیریت‌های عملی |
| `/promotions/codes` | کدهای تخفیف — جدول دسکتاپ (عنوان/کد/مقدار/استفاده‌شده/تاریخ/وضعیت/عملیات) + کارت موبایل + منوی اقدامات ردیف |
| `/promotions/codes/new` | کد تخفیف جدید — نوع/مقدار، تنظیمات عمومی، دامنهٔ اعمال (کل فروشگاه یا آکاردئون‌های محصول/دسته‌بندی/مشتری/موقعیت/سبد خرید) |
